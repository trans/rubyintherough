...
require 'singleton'
require 'facet/kernel'
require 'facet/binding'
require 'facet/binding/class'
...
if not defined? Cut
  ...
  class Class
    # this finds the "superest" cut which lacks the specified method
    def clearcut(meth)
      r = cuts.reverse.find{ |a| ! a.instance_methods.include?(meth.to_s) }
      #acuts = cut.ancestors.select{ |a| a.is_a?(Cut) }.reverse
      #r = acuts.detect{|a| ! a.instance_methods.include?(meth.to_s) }
      return r
    end
  ...
  module Weaving
    ...
    class << self
      ...
      def advice_applicable(aspect, klass, meth)
        ...
        return aspect.crosscheck?(wp) ? aspect.crosscut(wp) : nil
      end
      
      # apply an aspect to a given class#method
      def apply(aspect, klass, meth)
        return if klass.kind_of?(Cut)
        return if ADVICE_TABLE.has_key?( [aspect,klass,meth] )
        advice = advice_applicable(aspect, klass, meth)
        if advice
          c = klass.wrap( meth ) do |*args|
            # return super if Weaving.off?
            r = nil
            target=binding()  # overkill, should change (if we could get super's binding ?)
            target.call = proc{ |*args| r=super(*args) }  
            #target.call = proc{ |*args,&blk| r=super(*args,&blk) }  # for 1.9?
            s = aspect.send(advice, target, *args) #&blk)
            aspect.class.passive?(advice) ? r : s
          end
...
#           }
      ...
      def unapply(aspect, klass, meth)
        if c = ADVICE_TABLE[ [aspect, klass, meth] ]
          ...
          ADVICE_TABLE.delete( [aspect, klass, meth] )
        end
      end
