...
class Module
  ...
  def method_added(meth)
    TraceSet.active.each{|ts|
      if ( ts.respond_to?(:advice) &&
           ts.klass =~ self.name &&
           ts.meth =~ meth.to_s &&
           (ts.event =~ 'call' or ts.event =~ 'return') )
        c = self.cuts.detect{|c| ! c.instance_methods.include?(meth) }
        if ! c
          if self.cuts.empty?
            c = Class.new(self)
          else
            c = Class.new(self.cuts.last)
          end
          self.cuts << c
        end
        c.class_eval do
          ...
          define_method(meth) { |*args|
            send( "advice___#{meth}", proc{super(*args)}, *args) 
          }
          define_method("advice___#{meth}", ts.method(:advice).to_proc)
        end
      end
    }
  end
